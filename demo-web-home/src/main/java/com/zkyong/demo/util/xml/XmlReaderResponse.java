package com.zkyong.demo.util.xml;
...
public class XmlReaderResponse extends XmlParserBase{
	...
	public <T> T readXmlChild(Element parent, Class<T> clazz, Map<String, XmlMethods> methods) throws Throwable{
		if(parent == null){
			return null;
		}
		...
		while (it.hasNext()) {
			...
			if(java.util.List.class.isAssignableFrom(paraClass)){
				//list行记录
				if( xmlMethod.listGenericClazz == null ||
					xmlMethod.getMethod == null ||
				...
				Object row = readXmlChild(element, xmlMethod.listGenericClazz, xmlMethod.listXmlMethods);
				lists.add(row);
